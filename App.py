...
# from pyresparser import ResumeParser  # Broken - config issues
from working_parser import ResumeParser
...
# import pafy #for uploading youtube videos - deprecated
...
def fetch_yt_video(link):
    # Simple fallback since pafy is deprecated
    return "Resume/Interview Tips Video"
...
connection = pymysql.connect(host='localhost',user='root',password='Root12345',db='cv')
...
def run():
    ...
    link = '[©Developed by jaysingh](https://www.linkedin.com/in/jaysinghpatel/)'
    ...
    if choice == 'User':
        ...
        if pdf_file is not None:
            ...
            if resume_data:
                ...
                if 'Hobbies' in resume_text or 'Interests' in resume_text:
                    ...
                    st.markdown('''<h5 style='text-align: left; color: #000000;'>[-] Please add Hobbies. It will show your persnality to the Recruiters and give the assurance that you are fit for this role or not.</h5>''',unsafe_allow_html=True)
        ...
        if st.button('Login'):
            if ad_user == 'jaysingh' and ad_password == 'admin123':
                ...
                fig = px.pie(values=values.values, names=values.index, title='Predicted Field according to the Skills')
                st.plotly_chart(fig)
